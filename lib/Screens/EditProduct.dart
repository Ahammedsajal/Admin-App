...
class _EditProductState extends State<EditProduct>
  ...
  // Helper method to generate all combinations of attribute values
  void getCombination(List<String> currentAttr, List<String> currentId, int depth) {
    if (finalAttList.isEmpty) return;
    if (depth == finalAttList.length) {
      resultAttr.add(currentAttr.join(','));
      resultID.add(currentId.join(','));
      return;
    }
    for (var value in finalAttList[depth]) {
      getCombination(
...
/// Max variants you’ll allow (raise or lower to suit your UX)
static const int kVariantCap = 500;    

void buildCombinations() {
  variationList.clear();
  if (finalAttList.any((l) => l.isEmpty)) return;          // nothing to combine
  _walk([], [], 0);
}

/// depth-first walker
void _walk(List<String> names, List<String> ids, int depth) {
  // stop if we already generated “too many”
  if (variationList.length >= kVariantCap) return;

  for (final val in finalAttList[depth]) {
    final nextNames = [...names, val.value!];
    final nextIds   = [...ids,   val.id!   ];

    if (depth == finalAttList.length - 1) {
      // leaf – add a single variant
      variationList.add(
        Product_Varient(
          attr_name: nextNames.join(','),
          id:        nextIds.join(','),
        ),
      );
    } else {
      _walk(nextNames, nextIds, depth + 1);    // descend
    }

    if (variationList.length >= kVariantCap) return;        // early exit
  }
}
